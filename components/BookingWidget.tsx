...
export default function BookingWidget({
  ...
  const [selectedBedrooms, setSelectedBedrooms] = useState(
    defaultBedrooms ?? (disableBedroomSelection ? (maxBedrooms ?? 1) : 1)
  );
  const [dateWarning, setDateWarning] = useState("");
  const [currentMonth, setCurrentMonth] = useState(new Date());

  const currentPrice = disableBedroomSelection ? price : (pricePerBedroom ? selectedBedrooms * pricePerBedroom : price);
  ...
  const handleCheckAvailability = () => {
    ...
    const url = `https://wa.me/918234079482?text=${encodeURIComponent(message.join("\n"))}`;
    ...
    <div className="sticky top-24 bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
      ...
      <div className="space-y-4">
        ...
        {maxBedrooms && pricePerBedroom && (
          <div className={disableBedroomSelection ? "opacity-60" : ""}>
            <label className="block text-xs font-medium text-gray-700 uppercase tracking-wider mb-2">
              SELECT BEDROOMS
            </label>
            <div className="grid grid-cols-4 gap-2">
              {Array.from({ length: maxBedrooms }, (_, index) => index + 1).map((bedroomCount) => {
                const isSelected = selectedBedrooms === bedroomCount;
                return (
                  <button
                    key={bedroomCount}
                    type="button"
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">
              {disableBedroomSelection
                ? `₹${price.toLocaleString("en-IN")} total per night`
        ...
        <button
          ...
          className="w-full bg-foreground text-white py-4 rounded-lg font-medium hover:bg-foreground/90 transition-colors duration-200 mt-6 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          Send Inquiry
        </button>
        ...
        <p className="text-xs text-gray-600 text-center mt-3">
          or call us directly 📞{" "}
          <a href="tel:+918234079482" className="font-semibold text-gray-900 hover:underline">
            +91 82340 79482
          </a>
        </p>
      </div>
